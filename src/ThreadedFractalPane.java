...
public class ThreadedFractalPane extends JPanel implements ComponentListener, MouseListener, MouseMotionListener
{
	...
	private Stack<CoordinateConverter> converters = new Stack<CoordinateConverter>();
	...
	public ThreadedFractalPane()
	{
		...
		colorMap = new WeirdColorConverter();
		setImage();
    ...
    public Color calculateColorAt(int x, int y)
    {
		...
		for(int i=0;i<MAX_DEPTH;i++){
			if(z.compareTo(THRESHOLD)<0){
				z=z.squared().plus(c);
			}
			else{
				col = colorMap.colorFor(i);
				break;
			}
		}
		//-------------------------------
	...
	public void mouseReleased(MouseEvent e){
		...
		//converters.push(coordConverter.converterCopier());
		...
		if (minX < maxX && minY < maxY)
		{
			...
			else if(e.isShiftDown()&&converters.size()!=0){
				//TODO: My Crazy Stack Undo list idea
				coordConverter = converters.pop();
				calculationThreadNeedsReset = true;
				repaint();
			}
		}
